...
import { throwInvalidCredentialsError, findAuthWithUserBy, createProviderId, validateAndGetUserFields, sanitizeAndSerializeProviderData, createUser, rethrowPossibleAuthError, findAuthIdentity, deserializeAndSanitizeProviderData, doFakeWork, deleteUserByAuthId, updateAuthIdentityProviderData } from 'wasp/auth/utils';
import { invalidateSession, createSession } from 'wasp/auth/session';
import { TimeSpan, createJWT, validateJWT } from 'wasp/auth/jwt';
...
function ensureEnvVarsForProvider(envVarNames, provider) {
  const result = {};
...
function setOAuthCookieValue(provider, res, stateType, value) {
...
function getOAuthCookieValue(provider, req, stateType) {
  const cookieName = `${provider.id}_${stateType}`;
  const cookies = parseCookies(req.headers.cookie ?? "");
  return cookies.get(cookieName);
}

function generateAndStoreOAuthState(stateTypes, provider, res) {
  const result = {};
  ...
  if (stateTypes.includes("codeVerifier")) {
...
function validateAndGetOAuthState(stateTypes, provider, req) {
  const result = {};
  if (stateTypes.includes("state")) {
...
async function finishOAuthFlowAndGetRedirectUri(provider, providerProfile, providerUserId, userSignupFields) {
...
async function getAuthIdFromProviderDetails(providerId, providerProfile, userSignupFields) {
...
function createOAuthLoginHandler(provider, stateTypes, getAuthorizationUrl) {
...
function createOAuthCallbackHandler(provider, stateTypes, userSignupFields, getProviderInfo) {
  return handleRejection(async (req, res) => {
    try {
...
const getGoogleUserFields = defineUserSignupFields({
...
function getGoogleAuthConfig() {
  return {
    ...
    // must include at least 'profile' for Google
  };
}

const _waspUserSignupFields$1 = getGoogleUserFields;
...
const _waspConfig = {
  ...
  createRouter(provider) {
    ...
    return createOAuthProviderRouter({
      ...
      userSignupFields: _waspUserSignupFields$1,
